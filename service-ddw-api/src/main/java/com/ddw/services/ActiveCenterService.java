...
public class ActiveCenterService extends CommonService {
    ...
    public ResponseApiVO getList(String token, PageNoDTO dto)throws Exception{
        ...
        List list=this.commonList("ddw_activity","createTime desc","t1.dtTitle title,CONCAT_WS('至',DATE_FORMAT(t1.dtStartTime,'%Y-%m-%d'),DATE_FORMAT(t1.dtEndTime,'%Y-%m-%d')) activeTime,DATE_FORMAT(t1.createTime,'%Y-%m-%d %H:%i:%S') createTime,t1.dtImgPath imgUrl,t1.dtTargetPath jumpUrl",dto.getPageNo()==null?1:dto.getPageNo(),10,searchM);
