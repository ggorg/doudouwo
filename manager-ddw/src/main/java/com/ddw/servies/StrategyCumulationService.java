...
public class StrategyCumulationService extends CommonService {
    public Page findPage(Integer pageNo)throws Exception{
        Map condtion=new HashMap();
        return this.commonPage("ddw_strategy_cumulation","id asc",pageNo,10,condtion);
    }

    public Map getById(Integer id)throws Exception{
        return this.commonObjectBySingleParam("ddw_strategy_cumulation","id",id);
    }
    ...
    public ResponseVO saveOrUpdate(StrategyDTO strategyDTO)throws Exception{
        ...
        if(strategyDTO.getId() > 0){
            ...
            return super.commonUpdateBySingleSearchParam("ddw_strategy_cumulation",updatePoMap,"id",strategyPO.getId());
        }else{
            strategyPO.setCreateTime(new Date());
            strategyPO.setUpdateTime(new Date());
            return this.commonInsert("ddw_strategy_cumulation",strategyPO);
        }
    }
    ...
    public ResponseVO delete(int id){
        ...
        int n = this.commonDelete("ddw_strategy_cumulation","id",id);
        if(n>0){
            vo.setReCode(1);
            vo.setReMsg("删除成功");
        }else{
