...
public class PracticeGradeService extends CommonService{
    ...
    public Page findList(String gradeName)throws Exception{
        ...
        return super.commonPage("ddw_practice_grade","sort",1,999,condtion);
    }

    public GradePO selectById(String id){
        try {
            return super.commonObjectBySingleParam("ddw_practice_grade","id",id,new GradePO().getClass());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public ResponseVO saveOrUpdate(GradeDTO gradeDTO)throws Exception{
        ...
        if(gradeDTO.getId() > 0 ){
            ...
            vo=super.commonUpdateBySingleSearchParam("ddw_practice_grade",updatePoMap,"id",gradeDTO.getId());
        }else{
            vo=super.commonInsert("ddw_practice_grade",gradePO);
        }
    ...
    public ResponseVO delete(String id){
        ...
        int n = super.commonDelete("ddw_practice_grade","id",id);
        if(n>0){
            vo.setReCode(1);
            vo.setReMsg("删除成功");
        }else{
