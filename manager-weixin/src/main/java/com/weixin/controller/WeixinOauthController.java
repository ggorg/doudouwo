...
public class WeixinOauthController {
    ...
    public String oauth(String code, String state, HttpServletRequest request, RedirectAttributes attributes,HttpSession session,Model model) {
        ...
        try {
            ...
            if (code != null && code.length() > 0 && StringUtils.isNotBlank(state)) {
                ...
                if(json.containsKey("page")){
                    ...
                    if("obn".equals(page)){
                        ...
                        logger.info("h5商城{}",page);
                            ...
                            BaseTokenUtil.putUserIdAndStoreId(base64Token,userId,Integer.parseInt(params[0]),openid,ui.getNickname());
            ...
            logger.error("WeixinOauthController->oauth->跳转失败",e);
        }
        // 所有请求都跳转到此页面上
        return InternalResourceViewResolver.REDIRECT_URL_PREFIX + "/404";
    }
